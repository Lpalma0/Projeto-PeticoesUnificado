...
PATH_TEMPLATE_SENTENCA = Config.PATH_TEMPLATE_SENTENCA
PATH_INPUT_EXCEL_SENTENCA = Config.PATH_INPUT_EXCEL_SENTENCA
PATH_OUTPUT_SENTENCA = Config.PATH_OUTPUT_SENTENCA
...
class GeneratePetSentenca:
    ...
    def create_doc_word(self, process_number: str, name: str, valor: str):
        ...
        documento = Document(PATH_TEMPLATE_SENTENCA)
        ...
        documento.save(rf"{PATH_OUTPUT_SENTENCA}\{name_doc}")
        print("ok")
...
def start():
    ...
    requisicoes_simultaneas = int(
        input("Quantas requisições simultaneas deseja fazer?")
    )

    df = pd.read_excel(rf"{PATH_INPUT_EXCEL_SENTENCA}", dtype=str)

    numero_raw = (
        len(df) / requisicoes_simultaneas
    )  # < Número que deve ser trocado para divisão <
    ...
    for index, lista in enumerate(lista_separada):
        sleep(2)
        t = threading.Thread(
            target=GeneratePetSentenca().generate,
